...
def add_arguments_for_module(parser, module, argument_for_class, default, skip_params=[], parameter_defaults={}):
    ...
    argspec = inspect.signature(class_obj.__init__)
    params = argspec.parameters.values()
    defaults = [param.default for param in params if param.default is not param.empty][::-1]
    args = [param.name for param in params][::-1]
